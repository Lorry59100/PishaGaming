    ...
    if (decodedUserToken && !fetchDataExecuted.current) {
      ...
      const fetchData = async () => {
        try {
          ...
          console.error(
            "Erreur lors de la récupération du panier utilisateur :",
            error
          );
        }
      };
        ...
        axios
          ...
          .then((response) => {
            if (
              response.data &&
              response.data.carts &&
              JSON.stringify(response.data.carts) !== JSON.stringify(cart)
            ) {
              console.log("Updating cart from server:", response.data.carts);
              setCart(response.data.carts);
            }
          })
          .catch((error) => {
            console.error(
              "Erreur lors de la mise à jour du panier utilisateur :",
              error
            );
          });
      }
