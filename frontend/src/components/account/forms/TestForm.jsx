...
export function TestForm({ onClose, productId, testExist, onSubmit }) {
  ...
  useEffect(() => {
    ...
    if (testExist) {
      ...
      axios
        .get(`${URL}${URL_GET_TEST.replace(":id", productId)}`, {
        ...
        .then((response) => {
          ...
          setPros([
            ...positivePoints,
            ...Array(3 - positivePoints.length).fill(""),
          ]);
          setCons([
            ...negativePoints,
            ...Array(3 - negativePoints.length).fill(""),
          ]);
        })
        .catch((error) => {
          console.error(
            "Erreur lors de la récupération des données du test :",
            error
          );
        });
    }
  ...
  const submitTest = () => {
    ...
    if (testExist) {
      ...
      axios
        .put(`${URL}${URL_UPDATE_TEST.replace(":id", productId)}`, data, {
          headers,
        })
        .then(() => {
          handleClose();
      ...
      axios
        .post(`${URL}${URL_SUBMIT_TEST}`, data, { headers })
        .then(() => {
          handleClose();
          onSubmit();
        })
  ...
  return (
    <div className="test-form-container" ref={formRef}>
      ...
      <div className="test-form">
        <div className="avatar">
          {userData && userData.img ? (
            <img
              ...
              alt="User Image"
              className="user-img-circled-test-form"
            />
          ) : (
            <div>Loading...</div>
          )}
        ...
        <p>
          Afin d'orienter les autres utilisateurs, vous pouvez écrire votre
          propre opinion sur ce jeu, et dire ce que vous avez aimé ou non.
        </p>
