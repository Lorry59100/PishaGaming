...
export function PaymentForm() {
  ...
  const stripePromise = useMemo(
    () => loadStripe(import.meta.env.VITE_STRIPE),
    []
  );
  ...
  useEffect(() => {
    if (decodedUserToken) {
      ...
      axios
        .get(`${URL}${URL_GET_ADDRESS}`, { headers })
        .then((response) => {
  ...
  const handlePayClick = (event) => {
    ...
    if (!activeAddress) {
      ToastError(
        "Vous devez sélectionner une adresse de facturation/livraison"
      );
        ...
        <div className="payment-container">
          <div className="address-and-payform-container">
            <div className="address-container">
              ...
              <div
                className={`info-change-container ${
                  deliveryError ? "error-border" : ""
                }`}
              >
                {activeAddress ? (
                  <>
                    <div className="address-info address-info-pay">
                      <div className="address-info-container">
                        <h3>
                          {activeAddress.lastname} {activeAddress.firstname}
                        </h3>
                        <h4>
                  ...
                  <>
                    <div className="address-info address-info-pay-no-address">
                      <h4>
                        Vous n'avez pas d'adresse de facturation/livraison
                        renseignée.
                        <br /> Vous devez en avoir une pour passer commande.
                      </h4>
                      <button
                        className="submit-button create-address-pay"
                ...
                <div className="address-modal">
                  <div className="address-modal-content">
                    ...
                    {addresses.map((address) => (
                      <div
                        key={address.id}
                        className="single-address-container"
                      >
                        <h3>
                    ...
                    <div className="submit-address-container">
                      <button
                        className="submit-button add-address-btn"
                        onClick={handleOpenAddressForm}
                      >
                        <IconContext.Provider
                          value={{ size: "1.5em", color: "white" }}
              ...
              {cartData.carts &&
                ...
                cartData.carts.some((item) => item.isPhysical) && (
                  <div className="date-picker-container">
                    <h2>
                      Vous devez choisir une date de livraison pour les produits
                      suivants
                    </h2>
                    <div
                      className={`physical-products-container ${
                        dateError ? "error-border" : ""
                      }`}
                    >
                          ...
                          cartData.carts
                            .filter(
                              (item) =>
                                ...
                                (item.category === "Jeux Vidéos" &&
                                  item.isPhysical)
                            )
                            .map((item, index) => (
                              <div key={index} className="physical-product">
                                <img
                                  src={`${URL}${URL_MAIN_IMG}/${item.img}`}
                                  alt={item.name}
                                />
                                <h4 className="physical-product-name">
                                  {item.name}
                                </h4>
                                <div className="multiple-items">
                                  <div className="delivery-platform">
                                    <img
                                      src={`/src/assets/img/platforms/${item.platform}.png`}
                                      alt={item.platform}
                                      className="logo-img-delivery"
                                    />
                                  </div>
                                  {item.quantity > 1 && (
                      ...
                      {dateError && (
                        <div className="error-message">
                          <p>
                            Veuillez sélectionner une date de livraison pour les
                            produits physiques.
                          </p>
                        </div>
                      )}
            ...
            <div className="warning">
              ...
              <p>
                Ce site est actuellement en mode de démonstration. Veuillez ne
                pas renseigner vos vraies informations de paiement. Pour tester
                les fonctionnalités de paiement, utilisez le numéro de carte de
                test suivant :
                <br />
                <button
          ...
          <div className="summary-and-total-container">
            ...
            {cartData.carts &&
              cartData.carts.map((item, index) => (
                <div key={item.id} className="items-container">
                  <div className="single-item-container">
                    <div className="product-info-quantity">
                      <h3>
                        ...
                        <span
                          className={
                            item.quantity > 1 ? "multiple-items-info" : ""
                          }
                        >
                  ...
                  {index !== cartData.carts.length - 1 && (
                    <div className="cutline-summary-form"></div>
                  )}
                </div>
              ))}
            ...
            <span>
              <div className="terms-and-politic">
                En cliquant sur "Payer" je reconnais avoir lu et accepté
                les&nbsp;
                <a
                  href={URL_SALES_CONDITION}
                  className="pay-terms"
                  target="_blank"
