...
function Navbar() {
  ...
  useEffect(() => {
    if (isNeonTheme) {
      ...
      document.documentElement.style.setProperty(
        "--orange-gradient",
        "linear-gradient(10deg, #66c4ff, transparent) #4169e1"
      );
      document.documentElement.style.setProperty(
        "--orange-gradient-hover",
        "linear-gradient(190deg, #33b1ff, transparent) #66c4ff"
      );
      document.documentElement.style.setProperty(
        "--orange-logo-filter",
        "invert(56%) sepia(92%) saturate(500%) hue-rotate(180deg) brightness(104%) contrast(105%)"
      );
      document.documentElement.style.setProperty(
        "--orange-smooth-gradient",
        "linear-gradient(to right, transparent, #66c4ff, #009dff, #66c4ff, transparent)"
      );
    } else {
      // ORANGE
      ...
      document.documentElement.style.setProperty(
        "--orange-gradient",
        "linear-gradient(10deg, #ff8000, transparent) #ff4020"
      );
      document.documentElement.style.setProperty(
        "--orange-gradient-hover",
        "linear-gradient(190deg, #ff2020, transparent) #ff8000"
      );
      document.documentElement.style.setProperty(
        "--orange-logo-filter",
        "invert(56%) sepia(92%) saturate(1011%) hue-rotate(0deg) brightness(104%) contrast(105%)"
      );
      document.documentElement.style.setProperty(
        "--orange-smooth-gradient",
        "linear-gradient(to right, transparent, #ff8000, #ff5400, #ff8000, transparent)"
      );
    }
  }, [isNeonTheme]);
  ...
  return (
    ...
    isNavbarVisible && (
      <div
        className={`navbar ${
          showLoginAndRegisterForm ? "" : scrolled ? "scrolled" : ""
        } ${isVisible ? "visible" : "hidden"} ${isAtTop ? "at-top" : ""}`}
      >
        <div className="logo">
          <Link to={`${URL_HOME}`}>
        ...
        <div className="profile-container">
          <div className={`cart-profile ${userToken ? 'connected' : ''} ${itemCount === 0 ? 'empty-cart' : ''}`}>
            <IconContext.Provider value={{ size: "2em" }}>
              ...
              {itemCount > 0 && (
                <div className="cart-number-items-container">{itemCount}</div>
              )}
              {!decodedUserToken && (
              ...
              {decodedUserToken && userData && userData.img && (
                <button onClick={toggleProfileVisibility}>
                  <img
                    src={`${URL}${URL_IMG}/${userData.img}`}
                    alt="User Image"
                    className="user-img-circled-navbar"
                  />
                </button>
              )}
            </IconContext.Provider>
            {userToken && isProfileVisible && (
              <div
                className={`profile-content ${isNeonTheme ? "neon-theme" : ""}`}
                ref={profileContentRef}
              >
                <ul>
                  <li>
                    <Link
                      to={`${URL_ACCOUNT}${URL_PARAMETERS}`}
                  ...
                  <li>
                    <Link
                      to={`${URL_ACCOUNT}${URL_ORDER_HISTORIC}`}
                      onClick={handleLinkClick}
                    >
                      Achats
                    </Link>
                  ...
                  {decodedUserToken && (
                    <div className="user-panel-connected">
                      <li>
                        <Link to={URL_HOME} onClick={handleLogoutButtonClick}>
                          Déconnexion
                        </Link>
                      </li>
                  ...
                  <li className="neon-toggle-btn-container">
                    <button
                      ...
                      onClick={handleNeonThemeClick}
                    >
                      Thème
                      {isNeonTheme ? (
                        <MdToggleOn className="neon-theme-active-check" />
                      ) : (
                        <MdToggleOff className="theme-active-check" />
                      )}
                    </button>
                  </li>
                </ul>
            ...
            {!userToken && showLoginAndRegisterForm && (
