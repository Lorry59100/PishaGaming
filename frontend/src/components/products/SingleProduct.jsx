...
export function SingleProduct() {
  ...
  const { decodedUserToken, userToken } = useTokenService();
  const { updateCart } = useContext(CartContext);
  const navigate = useNavigate();
  const [showLoginAndRegisterForm, setShowLoginAndRegisterForm] =
    useState(false);
  ...
  const descriptionClassName = showFullDescription
    ? "big-description full"
    : "big-description";
  ...
  const getWishlist = useCallback(async () => {
    if (!userToken) {
      console.error("userToken is undefined");
      return;
    }
  ...
  }, [URL, URL_GET_WISHLIST, userToken]);
  ...
  const addToWishlist = async (productId, selectedPlatform) => {
    if (!userToken) {
      console.error("userToken is undefined");
      return;
    }
    ...
    console.log(
      "Adding to wishlist with productId:",
      productId,
      "and platform:",
      selectedPlatform
    );
    ...
    try {
      const response = await axios.post(
        `${URL}${URL_ADD_TO_WISHLIST.replace(":id", productId)}`,
        {
      ...
      if (response.data.success) {
        ...
        await getWishlist(); // Assurez-vous que getWishlist est appelé correctement
      } else {
  ...
  const addToCart = (redirect) => {
    if (!decodedUserToken) {
      ...
      const existingProductIndex = cart.findIndex(
        (item) => item.id === product.id && item.platform === selectedPlatform
      );
      if (existingProductIndex !== -1) {
        if (cart[existingProductIndex].quantity >= 10) {
          ToastCenteredWarning(
            "Vous ne pouvez pas ajouter plus de 10 exemplaires de ce produit."
          );
        } else {
      ...
      axios
        ...
        .then((response) => {
          if (response.data && response.data.success) {
            axios
              .get(`${URL}${URL_USER_CART}/${decodedUserToken.id}`)
              .then((cartResponse) => {
                if (
                  cartResponse.data &&
                  Array.isArray(cartResponse.data.carts)
                ) {
  ...
  const vote = (testId, isPositive) => {
    ...
    axios
      .post(`${URL}${URL_VOTE_TEST}${testId}`, data, { headers })
      .then((response) => {
  ...
  const updateTests = useCallback(() => {
    ...
    axios
      .get(`${URL}${URL_SINGLE_PRODUCT}/${id}`, { headers })
      .then((response) => {
  ...
  }, [id, userToken, URL, URL_SINGLE_PRODUCT]);

  useEffect(() => {
    updateTests(userToken);
  }, [updateTests]);
    ...
    <div className="single-product-container">
      ...
      {showLoginAndRegisterForm && (
        <LoginAndRegisterForm
          onCloseForm={() => setShowLoginAndRegisterForm(false)}
        />
      )}
      ...
      <div className="background-header-container">
        <div
          ...
          style={{
            backgroundImage: `url(${URL}${URL_MAIN_IMG}/${product.img})`,
          }}
        ></div>
      </div>
      ...
      <div className="dual-cards-container">
        <div className="single-product-img-container">
          <img
            src={`${URL}${URL_MAIN_IMG}/${product.img}`}
            alt={product.name}
          />
        </div>
          ...
          <div className="buy-info-container">
            <IconContext.Provider value={{ size: "2em" }}>
              <button
                ...
                onClick={() => addToWishlist(id, selectedPlatform)}
              >
        ...
        <div className="about-container">
          ...
          <a href="#desc" onClick={toggleDescription}>
            Voir plus
          </a>
        </div>
        ...
        <div className="users-tags">
          Tags utilisateurs*:
          {product.tags
            .slice(0, showAllTags ? product.tags.length : 4)
            .map((tag) => (
              <div key={tag.id} className="tags-list">
                <a href="/">{tag.name}</a>
              </div>
            ))}
          {!showAllTags && product.tags.length > 4 && (
            <div className="tags-list"></div>
          )}
          {product.tags.length > 4 && (
            <div
              className={showAllTags ? "show-less" : "show-more"}
              onClick={(e) => {
                e.preventDefault();
                setShowAllTags(!showAllTags);
              }}
            >
              <a href="/">{showAllTags ? "Moins" : "Plus"}</a>
            </div>
          )}
        </div>
        ...
        <div className="visuals-container">
          ...
          <div className="video-container">
            <video width="100%" height="700px" controls>
              <source
                src={`${URL}${URL_TRAILER}/${product.trailer}`}
                type="video/mp4"
              />
              <source
                src={`${URL}${URL_TRAILER}/${product.trailer}`}
        ...
        <div className={descriptionClassName}>
          ...
          <div className="big-desc-container">
            {showFullDescription
              ...
              : parseHTML(product.description, 200)}
          </div>
        </div>
              ...
              {product.tests
                ...
                .map((test) => (
                  <div
                    key={test.id}
                    className="test-item"
                    onClick={() => toggleComment(test.id, "most-rated")}
                  >
                    <div className="test-item-head">
                      <div className="publisher">
                        {test.avatar ? (
                          <img
                            src={`${URL}${URL_USER_AVATAR}${test.avatar}`}
                            alt="User Image"
                            className="user-img-circled"
                          />
                        ) : (
                          <IconContext.Provider value={{ size: "2em" }}>
                    ...
                    <h5>
                      {expandedComments[`most-rated-${test.id}`]
                        ? parse(test.commentaires)
                        : parseHTML(test.commentaires, 500)}
                    </h5>
                    <div className="points-container">
                      {test.points.filter((point) => point.isPositive).length >
                        0 && (
                        <div>
                          {test.points
                            ...
                            .map((point, index) => (
                              <div
                                key={index}
                                className="single-point-container"
                              >
                                <IconContext.Provider
                                  value={{ size: "1.2em", color: "green" }}
                                >
                                  <FaCheck />
                                </IconContext.Provider>
                    ...
                    <div className="test-item-foot">
                      ...
                      <div className="vote">
                        ...
                        <button
                          className={`${
                            test.hasVotedPositive ? "voted" : "thumb-up"
                          }`}
                          onClick={() => vote(test.id, true)}
                        >
                        ...
                        <button
                          className={`${
                            test.hasVotedNegative ? "voted" : "thumb-down"
                          }`}
                          onClick={() => vote(test.id, false)}
                        >
                          <IconContext.Provider value={{ size: "1.5em" }}>
                            <LuThumbsDown />
              ...
              {product.tests
                ...
                .map((test) => (
                  <div
                    key={test.id}
                    className="test-item"
                    onClick={() => toggleComment(test.id, "most-recent")}
                  >
                    <div className="test-item-head">
                      <div className="publisher">
                        {test.avatar ? (
                          <img
                            src={`${URL}${URL_IMG}/${test.avatar}`}
                            alt="User Image"
                            className="user-img-circled"
                          />
                        ) : (
                    ...
                    <h5>
                      {expandedComments[`most-recent-${test.id}`]
                        ? parse(test.commentaires)
                        : parseHTML(test.commentaires, 500)}
                    </h5>
                    <div className="points-container">
